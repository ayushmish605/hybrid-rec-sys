...
class TMDBDataLoader:
    ...
    def load_csv(self) -> pd.DataFrame:
        ...
        logger.info(f"Loading TMDB data from {self.csv_path}")
    ...
    def load_into_database(self, db: Session, update_existing: bool = False) -> Dict:
        ...
        log = ScrapingLog(
            source='tmdb_csv',
            movies_processed=self.stats['loaded'],
            reviews_found=0,
            status='completed',
            error_message=f"Skipped: {self.stats['skipped']}, Errors: {self.stats['errors']}"
        )
        db.add(log)
        db.commit()
